...
class MAVSDKAutoControl:
    ...
    async def land(self):
        ...
        for i in range(300):
            ...
            await self.drone.offboard.set_attitude(Attitude(0.0, 0.0, 0.0, thrust))
            await asyncio.sleep(0.2)
    ...
    async def test_mode(self):
        await self.drone.offboard.set_attitude(Attitude(0.0, 0.0, 0.0, 0.1))
...
class TrackingAndFocus:
    def __init__(self, device) -> None:
        ...
        self.source = cv2.VideoCapture(f"v4l2src device={device} ! video/x-raw, format = YUY2, width=640, height=480, framerate=30/1  ! videoconvert ! video/x-raw,format=BGR ! appsink")
